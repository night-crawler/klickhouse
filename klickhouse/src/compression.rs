...
type BlockReadingFuture<R> =
    Pin<Box<dyn Future<Output = Result<(Vec<u8>, &'static mut R, &'static mut Vec<u8>)>> + Send + Sync>>;
...
impl<'a, R: ClickhouseRead + 'static> AsyncRead for DecompressionReader<'a, R> {
    fn poll_read(
        ...
        while self.position >= self.decompressed.len() {
            ...
            let mode = self.mode;
            let alloc_buf: &'static mut Vec<u8> = unsafe { std::mem::transmute(self.buf.take().unwrap()) };
